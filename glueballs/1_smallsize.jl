...
smallsize = (JLD2.load("glueballs/smallsize.jld2"))["single_stored_object"]
# smallsize = Dict()
...
if false
    ...
    for N in Nlist
    smallsize[group][N] = Dict()

    d = locops[group][N]["localdim"]
    # Coupling loop
    λ_list = [1//10, 3//10, 5//10, 7//10, 9//10]

    for λ ∈ λ_list
    smallsize[group][N][λ] = Dict()

    # Small system size loop
    ls = [8]
    for l ∈ ls
    smallsize[group][N][λ][l] = Dict()
    dct = smallsize[group][N][λ][l]

    # Local operators
    dct["h"] = λ * locops[group][N]["Esq3"] - (1 - λ) * (locops[group][N]["Up3"] + locops[group][N]["Up3"]')

    # Operators
    dct["C"] = kron_power(SparseMatrixCSC(locops[group][N]["C1"]), l)
...
JLD2.save_object("glueballs/smallsize.jld2", smallsize)
...
if true
    ...
    for group in ["ZZ", "SU"]
        for N in [2,3]
            ...
            for ii in 1:ncols
                λ = λ_list[ii]
                Egs = smallsize[group][N][λ][l]["gsenergy"]
                Ebar = smallsize[group][N][λ][l]["centroid"]
                ΔE = Ebar - Egs
                Elist = smallsize[group][N][λ][l]["disprel"]["E"]
                Klist = smallsize[group][N][λ][l]["disprel"]["k"]
                Clist = smallsize[group][N][λ][l]["disprel"]["C"]
            ...
            Plots.savefig("Disprel$(group)$(N).pdf")
        end
    end
end